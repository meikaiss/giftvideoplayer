...
public class GLTextureView extends TextureView
    ...
    public void setEGLConfigChooser(int redSize, int greenSize, int blueSize, int alphaSize,
        ...
        setEGLConfigChooser(
                new ComponentSizeChooser(redSize, greenSize, blueSize, alphaSize, depthSize,
                        stencilSize));
    }
    ...
    private static class EglHelper {
        ...
        public boolean createSurface() {
            ...
            GLTextureView view = glTextureViewWeakRef.get();
            if (view != null) {
                eglSurface = view.eglWindowSurfaceFactory.createWindowSurface(egl, eglDisplay,
                        eglConfig,
                    ...
                    synchronized (glThreadManager) {
                        while (true) {
                            ...
                            if (paused != requestPaused) {
                                ...
                                if (LOG_PAUSE_RESUME) {
                                    Log.i("GLThread",
                                            "paused is now " + paused + " tid=" + getId());
                                }
                            }
                            ...
                            if (shouldReleaseEglContext) {
                                if (LOG_SURFACE) {
                                    Log.i("GLThread", "releasing EGL context because asked to " +
                                            "tid=" + getId());
                                }
                            ...
                            if (pausing && haveEglSurface) {
                                if (LOG_SURFACE) {
                                    Log.i("GLThread",
                                            "releasing EGL surface because paused tid=" + getId());
                                }
                                stopEglSurfaceLocked();
                            }
                            ...
                            if (pausing && haveEglContext) {
                                ...
                                if (!preserveEglContextOnPause
                                    ...
                                    stopEglContextLocked();
                                    if (LOG_SURFACE) {
                                        Log.i("GLThread", "releasing EGL context because paused " +
                                                "tid=" + getId());
                                    }
                                }
                            }
                            ...
                            if (pausing) {
                                if (glThreadManager.shouldTerminateEGLWhenPausing()) {
                                    ...
                                    if (LOG_SURFACE) {
                                        Log.i("GLThread",
                                                "terminating EGL because paused tid=" + getId());
                                    }
                                }
                            }
                            ...
                            if ((!hasSurface) && (!waitingForSurface)) {
                                if (LOG_SURFACE) {
                                    Log.i("GLThread",
                                            "noticed textureView surface lost tid=" + getId());
                                }
                                if (haveEglSurface) {
                                    stopEglSurfaceLocked();
                                }
                            ...
                            if (hasSurface && waitingForSurface) {
                                if (LOG_SURFACE) {
                                    Log.i("GLThread",
                                            "noticed textureView surface acquired tid=" + getId());
                                }
                            ...
                            if (readyToDraw()) {
                                ...
                                if (haveEglSurface) {
                                    if (this.sizeChanged) {
                                        ...
                                        if (LOG_SURFACE) {
                                            Log.i("GLThread", "noticing that we want render " +
                                                    "notification tid=" + getId());
                                        }
                            ...
                            if (LOG_THREADS) {
                                Log.i("GLThread",
                                        "waiting tid=" + getId() + " haveEglContext: " + haveEglContext
                                                + " haveEglSurface: " + haveEglSurface + " paused" +
                                                ": " + paused + " hasSurface: "
                                                + hasSurface + " surfaceIsBad: " + surfaceIsBad + " " +
                                                "waitingForSurface: "
                                                + waitingForSurface + " width: " + width + " " +
                                                "height: " + height
    ...
    private void checkRenderThreadState() {
        if (glThread != null) {
            throw new IllegalStateException("setRenderer has already been called for this " +
                    "instance.");
        }
    }
    ...
    private static class GLThreadManager {
        ...
        public boolean tryAcquireEglContextLocked(GLThread thread) {
            ...
            // TODOsuppress: implement a fairness policy. Currently
